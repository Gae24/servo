    ...
    Window: WindowPortsMethods + ?Sized,
{
    ...
    fn set_clipboard_content(&mut self, item: ClipboardItem) {
        if let Some(clipboard) = self.clipboard.as_mut() {
            let res = match item {
                ClipboardItem::Text(content) => clipboard.set_text(content),
                ClipboardItem::Html(content) => clipboard.set().html(content, None),
                ClipboardItem::Png(content) => clipboard.set_image(arboard::ImageData { width: 0, height: 0, bytes: content.into() }),
            };
            if let Err(e) = res {
                warn!("Error setting clipboard contents ({})", e);
            }
